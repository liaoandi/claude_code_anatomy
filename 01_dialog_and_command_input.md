...
`--version` 这样的 flag 会在最早的时机被处理，零模块加载直接退出。`daemon`、`bridge`、`ps/logs/attach/kill` 这些模式会在进入主交互循环之前就分流出去，走各自的初始化路径。只有普通交互模式才继续加载完整的主程序。

这种分流设计的好处是：不同运行模式的初始化要求差异很大，分流比统一处理要干净得多。冷启动时间也因此更可控——很多模块用动态 `import()` 懒加载，不需要时根本不加载。
