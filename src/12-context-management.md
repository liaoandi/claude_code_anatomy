...
2. **自动**：当上下文用量接近模型上限时，系统自动触发一次 compact，然后继续对话。你会在消息流里看到一条提示，说明刚才发生了压缩——不是悄悄进行的。如果不想要自动 compact，可以在设置里关掉。
...
Context collapse 没有用户命令，完全自动触发：系统在每次 API 调用前判断是否需要折叠，也会在遇到上下文溢出错误时自动触发。启用 context collapse 后，自动 compact 会被关掉——两者解决同一个问题，不会同时运行。手动 `/compact` 不受影响，仍然可用。
